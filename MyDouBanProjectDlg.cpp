...
BEGIN_MESSAGE_MAP(CMyDouBanProjectDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_STATIC_RECORDS, OnStaticRecords)
	ON_BN_CLICKED(IDC_STATIC_PAGES, OnStaticPages)
	ON_WM_KEYDOWN()
	ON_WM_CANCELMODE()
	ON_WM_CAPTURECHANGED()
	ON_CBN_EDITCHANGE(IDC_COMBO_QUERY_WAY, OnEditchangeComboQueryWay)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CMyDouBanProjectDlg::OnBtnQuery() 
{
	...
	{
		Type = SEARCH_KEY;	//关键字	key查询
	}
	else if (m_ComboBoxWay.GetCurSel() == 1)//选中1，即为"tag"标签查询
	{
		Type = SEARCH_TAG;	//标签		tag查询
	}

	((CEdit *)GetDlgItem(IDC_EDIT_QUERY))->GetWindowText(Query);
	if ( Query.IsEmpty() )	//为空
	{
...
void CMyDouBanProjectDlg::OnStaticRecords() 
{
	// TODO: Add your control notification handler code here
	
}

void CMyDouBanProjectDlg::OnStaticPages() 
{
	// TODO: Add your control notification handler code here
	
}

void CMyDouBanProjectDlg::OnEditchangeComboQueryWay() 
{
	// TODO: Add your control notification handler code here
